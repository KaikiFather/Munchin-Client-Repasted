...
namespace MunchenClient.Wrappers
{
	internal static class GeneralWrappers
	{
		...
		*/
		...
		internal static void ApplyPlayerWallhack(Renderer renderer, bool state, bool isFriend)
		{
			if (state)
			{
				if (isFriend)
				{
					PlayerHandler.EnableFriendESPState(renderer);
				}
				else
				{
					PlayerHandler.EnableStrangerESPState(renderer);
				}
			}
			else
			{
				PlayerHandler.DisableESPForPlayer(renderer);
			}
		}

		internal static void ApplyPlayerWallhack(bool state, PlayerInformation player)
		{
			if (!player.isLocalPlayer)
			{
				ApplyPlayerWallhack(player.vrcPlayer.field_Internal_PlayerSelector_0.GetComponent<Renderer>(), state, player.IsFriends());
			}
		}

		internal static void ApplyAllPlayerWallhack(bool state)
		{
			foreach (System.Collections.Generic.KeyValuePair<string, PlayerInformation> playerCaching in PlayerUtils.playerCachingList)
			{
				...
				{
					ApplyPlayerWallhack(playerCaching.Value.vrcPlayer.field_Internal_PlayerSelector_0.GetComponent<Renderer>(), state, playerCaching.Value.IsFriends());
				}
			}
		}
		...
		private static IEnumerator AlertPopupEnumerator(string title, string text, float timeout)
		{
			...
			{
				ConsoleUtils.Exception("GeneralWrappers", "AlertPopup", e, "AlertPopupEnumerator", 241);
			}
		}
		...
		internal static void AlertAction(string title, string content, string button1Text, System.Action button1Action, string button2Text, System.Action button2Action)
		{
			...
			{
				ConsoleUtils.Exception("GeneralWrappers", "AlertAction", e, "AlertAction", 255);
			}
		}
		...
		internal static void CloseMenu()
		{
			...
			{
				ConsoleUtils.Exception("GeneralWrappers", "CloseMenu", e, "CloseMenu", 267);
			}
		}
		...
		internal static void ClosePopup()
		{
			...
			{
				ConsoleUtils.Exception("GeneralWrappers", "ClosePopup", e, "ClosePopup", 279);
			}
		}
